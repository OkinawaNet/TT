...
5. Добавить новую версию события в schema registry. Это подготовительный шаг изначального списка.  
...
9. Добавить логику разделения старой строки `title` на `title` и `type` в сервисе найма. В коде это будет выглядеть так:  
